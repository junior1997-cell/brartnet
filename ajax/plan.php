        ...
        foreach ($rspta['data'] as $key => $value) {
          ...
          $data[] = array(
            ...
            "1" => '<button class="btn btn-icon btn-sm btn-warning-light" onclick="mostrar_plan(' . $value['idplan'] . ')" data-bs-toggle="tooltip" title="Editar"><i class="ri-edit-line"></i></button>'.
              ' <button  class="btn btn-icon btn-sm btn-danger-light product-btn" onclick="eliminar_plan(' . $value['idplan'] . ', \'' . encodeCadenaHtml($value['idplan']) . '\')" data-bs-toggle="tooltip" title="Eliminar"><i class="ri-delete-bin-line"></i></button>',         
            "2" => $value['nombre'],
            "3" => $value['costo'],
